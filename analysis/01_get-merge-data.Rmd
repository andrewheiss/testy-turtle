...
```{r show-donors}
aid_donors %>% datatable()
```

### List of recipients

```{r show-recipients}
select(aid_recipients, recipient) %>% datatable()
```

### List of purposes

```{r show-purposes}
arrange(aid_purposes, desc(n)) %>% datatable()
```
...
```{r final-data-missing-donors, fig.width=6, fig.height=3, fig.asp=NULL}
...
```{r data-missing-countries, fig.width=6, fig.height=7, fig.asp=NULL}
...
```{r missing-soviet-things, indent="    "}
...
```{r final-data-missing-countries, fig.width=6, fig.height=7, fig.asp=NULL}
country_aid_no_lags %>% 
  filter(laws) %>% 
  gg_miss_var(., show_pct = TRUE) +
  theme_donors()

...
- The within-country $\sigma$ for the $\mu$ part of the model: $\sigma_y$
...
We can see what this looks like with actual dollar amounts by using `linpred_draws(transform = TRUE)`. This will calculate predicted values from the $\operatorname{logit}(\pi_{it})$ part of the model
...
We can work with both of these parts simultaneously if we `epred_draws()` instead of `linpred_draws()`, which calculates the expectation (the "e" in epred) of the posterior predictive distribution of the outcome, or $\textbf{E}(\text{Foreign aid}_{it_j})$.
...
Here's what all three of these plots look like simultanously (just the $\pi$ hurdling part, just the $\mu$ non-hurdled part, and the combined epred part):
...
df_country_aid_laws %>% 
  ...
  scale_y_continuous(labels = label_percent()) +
  scale_color_manual(values = clrs$Prism[c(3, 11)]) +
  labs(x = NULL, y = "Proportion of countries with zero contentious aid",
       color = NULL) +
  theme_donors() +
  theme(legend.text = element_markdown())
```
...
p3 <- m_purpose_prelim_time_only_total$model %>% 
  ...
  labs(x = NULL, y = "Predicted proportion of\ncountries with 0% contentious aid",
       subtitle = "Predicted global proportion of countries receiving no contentious aid, π only") +
  theme_donors()

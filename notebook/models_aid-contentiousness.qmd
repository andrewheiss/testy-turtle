...
The max weight here is `r round(max(df_purpose_iptw_total$iptw), 0)`, but there are only a handful of observations with weights that high. The average is `r round(mean(df_purpose_iptw_total$iptw), 2)` and the median is `r round(median(df_purpose_iptw_total$iptw), 2)`, which are both close to 1, so we're good.
...
The max weight here is `r round(max(df_purpose_iptw_advocacy$iptw), 0)` which is lovely. The average is `r round(mean(df_purpose_iptw_advocacy$iptw), 2)`, so we're good.
...
The max weight here is `r round(max(df_purpose_iptw_entry$iptw), 0)` which is grand. The average is `r round(mean(df_purpose_iptw_entry$iptw), 2)`, so we're golden.
...
The max weight here is `r round(max(df_purpose_iptw_funding$iptw), 0)` which is fine. The average is `r round(mean(df_purpose_iptw_funding$iptw), 2)`, so all is well.
...
lol these blow up again. The max weight is `r label_comma()(max(df_purpose_iptw_ccsi$iptw))`, which, oh no.
...
We care most about conditional effects. That's what our main estimand is—the effect of treatment when all offsets are set to 0. 

::: {.panel-tabset}
#### Total barriers

In general, contentious aid doesn't really change as additional legal barriers are added:
...
The slope of this line is tiny across the whole range of de jure restriction.
...
Contentious aid *increases* in response to restrictions on advocacy.
...
Contentious aid *increases* in response to restrictions on entry.
...
Contentious aid *decreases* in response to restrictions on funding.
...
Contentious aid doesn't do much in response to changes in the de facto civil society environment.
...
Contentious aid *increases* as repression decreases/improves.
...
- $\sigma_0$ (`sd__(Intercept)` for the `gwcode` group): the variability between countries' baseline averages, or the variability around the $b_{0_j}$ offsets. Here it's `r round(sigmas_zi$sd_gwcode_intercept$estimate, 2)`, which means that across or between countries, the proportion of contentious aid by 0.5ish logit units.
